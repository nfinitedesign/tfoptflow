...
class FlowDroNetDataset(OpticalFlowDataset):
    """FlowDroNet optical flow dataset.
    """
    ...
    def set_folders(self):
        ...
        """
        self._trn_dir = f"{self._ds_root}/{self.opts['train_imgs']}"
        self._val_dir = self._trn_dir
        self._tst_dir = f"{self._ds_root}/{self.opts['test_imgs']}"

        self._trn_lbl_dir = f"{self._ds_root}/{self.opts['train_flow']}"
        self._val_lbl_dir = self._trn_lbl_dir
        self._val_pred_lbl_dir = f"{self._ds_root}/train_flow_pred"
        self._tst_pred_lbl_dir = f"{self._ds_root}/test_flow_pred"
    ...
    def _build_ID_sets(self):
        ...
        self._IDs = []
        folders = get_experiment_folders(self._trn_dir)
        for experiment in folders:  # video: 'alley_1'
            frames = sorted(os.listdir(experiment + '/images/'))
            for idx in range(len(frames) - 1):
                frame1_ID = f'{experiment[len(self._trn_dir) + 1:]}/images/{frames[idx]}'
                frame2_ID = f'{experiment[len(self._trn_dir) + 1:]}/images/{frames[idx+1]}'
                flow_ID = f'{experiment[len(self._trn_dir) + 1:]}/images/liteflownet-ft-kitti-{str(idx).zfill(7)}.png'
        ...
        self._tst_IDs = []
        folders = get_experiment_folders(self._tst_dir)
        for experiment in folders:  # video: 'alley_1'
            frames = sorted(os.listdir(experiment + '/images/'))
            for idx in range(len(frames) - 1):
                frame1_ID = f'{experiment[len(self._trn_dir) + 1:]}/images/{frames[idx]}'
...
def get_experiment_folders(folder_in):
    ...
    """
    if os.path.isdir(os.path.join(folder_in, 'images')):
        folders = [folder_in]
    else:
        tmp = next(os.walk(os.path.join(folder_in, '.')))[1]
        folders = []
        for f in tmp:
            folders.append(os.path.join(folder_in, f))
