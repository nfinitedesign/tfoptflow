...
class ModelFlowDroNet(ModelPWCNet):
    def __init__(self, name='flowdronet', mode='train', session=None, options=_DEFAULT_FLOWDRONET_OPTS, dataset=None):
    ...
    def flowdronet_predict(self, img_pairs):
        ...
        """
        These operations are necessary to keep the flow similar to training data.
        Centering is done to remove the steering bias and rescaling to compensate for different flow magnitudes.
        These operations could precede the DroNet network as tf ops (pseudo tf-code):
        mx = tf.reduce_mean(flow(x))
        my = tf.reduce_mean(flow(y))
        flow(x) = tf.add(flow(x), -mx)
        flow(y) = tf.add(flow(y), -my)
        ...
        flow = flow / np.mean(flow_magnitude)

        # Predict DroNet output from flow input if possible
        with self.dronet_graph.as_default():
            steer_coll = self.dronet_sess.run(self.dronet_y_tnsr, feed_dict={self.dronet_x_tnsr: flow})
